...
리뷰 과정에서 발견된 사항을 수정함: `Routine`의 `habitIds`/`order` 중복 필드 제거. 또한 `reminderTime`은 습관 삭제 시 orphan 데이터가 남지 않도록 `UserSettings`의 별도 Map이 아니라 `Habit`에 직접 귀속시킴.
...
3. **Home** — 오늘의 체크리스트: 날짜 헤더, 진행률 링 카드, 습관 카드(스트릭 뱃지, 탭 체크 토글), 빈 상태 유도 화면, FAB
4. **Habit Detail** — 스트릭/완료율 스탯, 최근 14일 도트 히스토리, 보관/삭제
5. **Weekly Report** — 요일별 완료율 바 차트, 주 평균, 주간 습관 별도 집계
6. **Settings** — 테마(시스템/라이트/다크), 알림 토글
...
- 수용 기준: 예정된 날을 놓치면 스트릭이 정상적으로 리셋됨
...
**F6. 히트맵 캘린더** — 월 단위 완료 여부를 색상 농도로 표시
**F7. 주간 리포트** — 완료율 추이 라인/바 차트
